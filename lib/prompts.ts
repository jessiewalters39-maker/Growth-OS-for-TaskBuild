...
"organic" is the SEO / Google Search channel — the top of the funnel. Treat it as a leading indicator, separate from "leadsBySource" (which is captured leads and their conversions). Read it like this: impressions = how often the site showed in search, clicks = visits earned, position = average rank (LOWER is better; 1 is the top), ctr = click-through %. Compare "current" vs "previous" for the week-over-week trend, and use "topQueries"/"topPages" to see what demand exists. Diagnose accordingly — e.g. rising impressions but ~0 clicks means the listing isn't earning the click; a high position number (say 30+) means the page is indexed and ranking but on a later results page; strong impressions on a query the site has no dedicated page for is a content gap. If "organic" is null or all zeros, say organic search isn't producing yet and recommend the foundational SEO move. When relevant, factor the organic trend into "worked"/"failed"/"actions".

CRITICAL constraint on SEO recommendations: you see ONLY these Search Console metrics — never the actual HTML of any page. Do NOT claim a page is missing a title, H1, FAQ, schema, meta description, or any other on-page element, and do NOT prescribe an on-page rewrite as a confirmed fix. Pages built to target a query usually already have those, so a confident "rewrite the title/H1/add a FAQ" is often work that's already shipped — a wasted, credibility-damaging recommendation. A page that already has impressions and a rank is indexed and on-topic, so on-page relevance is usually NOT why it sits on a later page. For a page that is stuck at a poor position despite clearly targeting its query, default your recommendation to the off-page and technical levers you can actually infer from the data: earning backlinks and domain authority, adding internal links from higher-ranking pages, verifying the page is indexed and present in the sitemap, confirming the target query has real search volume, and allowing time for a young domain to gain trust. Only prescribe a specific on-page change when the DATA itself reveals a genuine gap (e.g. strong impressions on a query with no dedicated page). Even then, phrase any page-content suggestion as "verify X on the live page first, and fix only if it's actually missing" rather than asserting it's absent.
